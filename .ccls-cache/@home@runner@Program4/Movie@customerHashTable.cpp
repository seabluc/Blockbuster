...
CustomerHashTable::~CustomerHashTable() {
  for (int i = 0; i < hashSize; i++) {
    ...
    while (crawler != nullptr) {
      ...
      /* vector is a non-pointer member so it doesnt allocate any memory; therefore
         delete rem should suffice. Will look into this is memory leaks occur.
      */
...
bool CustomerHashTable::addCustomer(Customer &customer) {
  ...
  if (table[index] != nullptr) {
    ...
    // Index not occupied, place the customer into directly into hashed index
    table[index] = ins;
    return true;
  }
}
